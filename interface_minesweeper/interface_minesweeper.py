...
def read_click(m,n):
    global x,y,cmd, bomb_count
    x.set(m)
    y.set(n)
    cmd.set('o')
    if buttons[m][n]['text']==flag:
...
def create_grid_gui(root, grid, start_function): #will receive the state_grid generated by the game code
    global  buttons, mainFrame, bomb, replay, rb, x,y
    ...
    # rb = Button(mainFrame, text=replay,font= ('Arial 12 bold'),
    #             command=start_function, bg='yellow')
    rb = Button(mainFrame, text=replay,font= ('Arial 12 bold'),
                command=lambda x=start_function: reset_game(x), bg='gray') #, fg='yellow')
    ...
    #end_label= Label(mainFrame, text = 'Bombs:', font= ('Helvetica 10'))
